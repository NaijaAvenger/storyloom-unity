...
namespace Storyloom
{
    ...
    public class PlayerController2D : StoryloomPlayer
    {
        ...
        Rigidbody2D _rb; Vector2 _facing = Vector2.down, _wanted;
        ...
        void Awake()
        {
            _rb = GetComponent<Rigidbody2D>(); _rb.gravityScale = 0; _rb.freezeRotation = true;
            // A body that falls asleep stops generating trigger enter/exit, which is how zone arrivals went missing after the
            // player stood still; interpolation + continuous detection keep the collider from tunnelling at run speed.
            _rb.sleepMode = RigidbodySleepMode2D.NeverSleep;
            _rb.interpolation = RigidbodyInterpolation2D.Interpolate;
            _rb.collisionDetectionMode = CollisionDetectionMode2D.Continuous;
            if (keys == null) keys = StoryloomDirector.Instance ? StoryloomDirector.Instance.keys : StoryloomKeyBinds.Default();
        }
        ...
        void Update()
        {
            ...
            float reach = Reach;
            Interactable near = Interactable.Nearest(origin, _facing, out float nd); NearestDistance = nd; NearestName = near ? near.name : "";
            var best = Interactable.Nearest(origin, _facing, out _, reach);   // range is applied while choosing, not afterwards
            if (best == null)   // physics fallback (colliders on any layer), nearest hit wins
            {
                float bestD = float.MaxValue;
                foreach (var h in Physics2D.OverlapCircleAll(origin, reach, interactMask))
                {
                    var it = h ? h.GetComponentInParent<Interactable>() : null;
                    if (it == null || !it.enabled || !it.gameObject.activeInHierarchy) continue;
                    float d = it.DistanceTo(origin, false); if (d < bestD) { bestD = d; best = it; }
                }
            }
            SetFocus(best);
            HandleActionKeys();
        ...
        // Velocity belongs in the physics step: driving it from Update meant the value the solver used depended on how many
        // render frames happened to land between two FixedUpdates, which is what made contacts and zone triggers feel random.
        void FixedUpdate() { _rb.velocity = _wanted; }
