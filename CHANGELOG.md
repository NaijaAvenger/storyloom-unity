...
- **Event router** (StoryloomEventRouter): drop it in a scene, right-click ▸ “Sync rows from story”, and every event name gets a row with its own UnityEvent — wire cutscenes/unlocks in the inspector instead of switching on strings; code can subscribe per event with router.On(name, handler).
- **Journal / codex on J**: the reserved journal key now opens a four-column codex — people met (role, faction, relationships, current mood), places visited (with descriptions), lore learned, items held — all read from state the runner already tracks. Counts as a modal (movement/look pause, cursor freed) alongside the inventory via the new director.UiBusy.
- **Objective line**: a HUD hint (“→ Talk to Bram”, “→ Go to the Harbour”) derived from the paused pending beat or the first currently-available one — the gating knowledge the director always had, now persistent on screen.
- **Ambient barks**: NPCs with idle lines speak them as small world-space bubbles when the player walks near (per-NPC cooldown + scene-wide gap; once-only lines are saved for real conversations and visit counts are not consumed). Added to generated scenes and to existing ones by the runtime auto-repair.

## 0.9.0
- **Key bind editor in the window** (“Edit key binds” foldout under the help line): click a bind, the window listens for the next key press, Apply commits (Cancel or another bind aborts). Defaults stay visible per row with a “(changed)” marker. Rebinding to a key another action holds prompts — confirming **swaps**, giving the other action this bind’s old key so no action is left dead. Applies live (Rebuild) so a running play session picks changes up; gamepad bindings remain fixed.
- **Sync review + no more duplicates.** A live-link pull now opens a review window before anything is applied: every change (added / removed / edited characters, items, locations, nodes) has a checkbox to bring it in or ignore it this pull. Entities whose id changed upstream (the source of “characters listed multiple times” — every pull looked like remove-all + add-all) are detected by name and offered as **migrations**: your binding rows and entity assets are re-pointed at the new id in place, assignments intact, instead of duplicating. Accepted removals prune their binding rows; stale unassigned rows and double rows from earlier silent pulls are cleaned up in the same pass. Ignoring changes rewrites the local copy from Unity’s data model (unparsed export fields drop until the next fully-accepted pull — stated in the window).
...
## 0.8.0
- **Ported the site-side export features** (from the storyloom.com kit build): **behavior tags** (export v2.2 — named switches decisions can flip; nodes/options carrying `behaviorTagIds` are only available while every tag is active, effects `enable`/`disable`/`toggle` flip them, and the director/simulator gate beats on them); **new condition namespaces** usable everywhere conditions appear — `loc:<id>` / `region:<id>` (`visited`, `not visited`, `currently in`, `not in`; the director marks locations and their region chain visited in `SetLocation` via `Runner.VisitLocation`), `lore:<id>` (`knows` / `doesn't know`, set by `learn`/`forget` effects) and `mood:<characterId>` — all living in the runner's variable store so saves carry them automatically; and **idle / revisit lines** (`Character.barks`, picked by `StoryloomDirector.PickBark`: conditional lines first, then exact-visit lines, then general lines by priority, cycling ties so repeats vary; visit counts and once-only lines are saved/loaded and reset with `ResetStory`). New runner tests cover tags and location/lore conditions.
- **Create test scene is safe from a non-blank editor**: it refuses to run during play mode, and prompts to save the currently open modified scene(s) before building (Cancel aborts) instead of silently discarding them; Create/Generate/Placeholder buttons are disabled while playing.
- **Post-build verification pass**: after generation (and its automatic repair), the scene is checked end-to-end — director/bindings, player body + style match, camera driver, CursorLock, EventSystem, dialogue/toast/inventory/banner presence *and* internal wiring, zone colliders (dimension, trigger flag, kinematic rigidbody), and interactables with wrong-dimension colliders. All-clear logs one line; problems are listed in the Console and counted in the window notification.

## 0.7.0
- **Custom UI: the director now talks to its UI through interfaces.** `IDialogueUI`, `ILocationBannerUI`, `IPickupToastUI`, `IInventoryUI` (`StoryloomUIContracts.cs`, contract notes included); the kit's uGUI widgets are the default implementations. New "Custom UI" override slots on the director — drop in any component implementing the matching interface and the built-in widget is ignored; `ResolveUI` also adopts a custom implementation found in the scene when no built-in widget exists. Every call site (director, players, interactables, debug HUD, self-test) goes through the interfaces.
- **`SpeechBubbleUI`** — a complete working `IDialogueUI`: billboarded text bubble over whoever is speaking (narration over the player), typewriter, nav-keys + advance choice list. Deliberately the smallest real implementation of the contract; copy it as the starting point for your own presenter.
- **Update check**: once per editor start, the installed version is compared against the repo's main branch; a prompt appears when a newer version exists, with a checkbox to silence itself (manual checks stay available: `Window ▸ Storyloom Check for Updates`, or the button in the Welcome window). Non-blocking, silent when offline.
- **Test suite**: `Tests/Editor/StoryloomRunnerTests.cs` — runner traversal (choice / check / random / jump), conditions and effects, the `item:` inventory convention, snapshot/restore round-trips, JSON parsing, and simulator ground truths (a story with a known soft-lock must report it; a clean one must not; both choice branches explored; orphan endings flagged; strict-order islands). Add `"testables": ["com.storyloom.unity"]` to the project manifest to see them in the Test Runner.
- The package now ships assembly definitions (`Storyloom.Runtime`, `Storyloom.Editor`, `Storyloom.Tests`). Code in `Assets/` keeps compiling unchanged (predefined assemblies auto-reference asmdefs); projects using their own asmdefs can now reference `Storyloom.Runtime` explicitly, which was impossible before.
- **Window fits narrow layouts**: the toolbar buttons flow into as many rows as the width needs instead of demanding a very wide window; button actions run after OnGUI (delayCall), which also fixes the "EndLayoutGroup: BeginLayoutGroup must be called first" console error that opening windows / rebuilding assets mid-layout produced. "(Stardew)" dropped from the style label and the generated scene name ("Top-down kit").
- **"UI always needs repairing" fixed at both ends**: a play-mode `ResolveUI` re-link can't be saved, so the same warning repeated every play — it now says exactly that and how to fix it, and out of play mode the re-link is persisted. *Repair open scene* now saves the scene as part of the action (a repair that wasn't saved silently reverted), and the banner upgrade reuses its existing Description child instead of adding a duplicate on every unsaved repair.
